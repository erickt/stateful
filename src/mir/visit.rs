...
use data_structures::tuple_slice::TupleSlice;
...
macro_rules! make_mir_visitor {
    ($visitor_trait_name:ident, $($mutability:ident)*) => {
        pub trait $visitor_trait_name {
            ...
            fn visit_mir(&mut self, mir: & $($mutability)* Mir) {
                self.super_mir(mir);
            }
            ...
            */
            ...
            fn visit_projection(&mut self,
                                lvalue: & $($mutability)* LvalueProjection,
                                context: LvalueContext,
                                location: Location) {
            ...
            */
            ...
            fn visit_ty(&mut self,
                        ty: & $($mutability)* P<ast::Ty>) {
                self.super_ty(ty);
            }

            /*
            fn visit_substs(&mut self,
                            substs: & $($mutability)* &'tcx Substs) {
                self.super_substs(substs);
            }

            fn visit_closure_substs(&mut self,
                                    substs: & $($mutability)* ClosureSubsts) {
            ...
            */
            ...
            fn visit_local_decl(&mut self,
                                local_decl: & $($mutability)* LocalDecl) {
            ...
            fn super_statement(&mut self,
                ...
                match *kind {
                    ...
                    StatementKind::Let {
                        pat: _,
                        ref $($mutability)* ty,
                        ref $($mutability)* lvalues,
                        ref $($mutability)* rvalue,
                    } => {
                        if let Some(ref $($mutability)* ty) = *ty {
                            self.visit_ty(ty);
                        }
                    ...
                    StatementKind::Call { ref $($mutability)* func,
                        ...
                        for arg in args {
                            self.visit_rvalue(arg, location);
                        }
                        self.visit_lvalue(destination, LvalueContext::Call, location);
                    }
                    StatementKind::MethodCall { ident: _,
                                                ref $($mutability)* tys,
                                                ref $($mutability)* self_,
                                                ref $($mutability)* args,
                                                ref $($mutability)* destination } => {
                        for ty in tys {
                            self.visit_ty(ty);
                        }
                        self.visit_operand(self_, location);
                        for arg in args {
                            self.visit_rvalue(arg, location);
                        }
                        self.visit_lvalue(destination, LvalueContext::Call, location);
            ...
            fn super_terminator_kind(&mut self,
                ...
                match *kind {
                    ...
                    TerminatorKind::Match { ref $($mutability)* discr,
                                            ref targets } => {
                        self.visit_operand(discr, source_location);
                        for target in targets {
                            self.visit_branch(block, target.block);
                        }
                    }
                    ...
                    */
                    ...
                    TerminatorKind::Suspend {
                        destination: (ref $($mutability)* destination, target),
                        ref $($mutability)* rvalue,
                    } => {
                        self.visit_lvalue(destination, LvalueContext::Call, source_location);
                        self.visit_branch(block, target);
                        self.visit_rvalue(rvalue, source_location);
                    }
                }
            }
            ...
            */
            ...
            fn super_rvalue(&mut self,
                ...
                match *rvalue {
                    ...
                    */
                    ...
                    Rvalue::Ref(mutability, ref $($mutability)* path) => {
                        self.visit_lvalue(path, LvalueContext::Borrow(mutability), location);
                    }
                    ...
                    */
                    ...
                    Rvalue::Tuple(ref $($mutability)* operands) => {
                        for operand in operands {
                            self.visit_operand(operand, location);
                        }
                    }
                    Rvalue::Struct(_, _, ref $($mutability)* operands, ref $($mutability)* with) => {
                        for operand in operands {
                            self.visit_operand(operand, location);
                        }
                        if let Some(ref $($mutability)* with) = *with {
                            self.visit_operand(with, location);
                        }
                    }
                    Rvalue::Range(ref $($mutability)* from, ref $($mutability)* to, _) => {
                        if let Some(ref $($mutability)* from) = *from {
                            self.visit_operand(from, location);
                        }
                        if let Some(ref $($mutability)* to) = *to {
                            self.visit_operand(to, location);
                        }
                    }
            ...
            fn super_projection(&mut self,
                                proj: & $($mutability)* LvalueProjection,
                                context: LvalueContext,
                                location: Location) {
                let Projection {
                    ref $($mutability)* base,
                ...
                let context = if context.is_mutating_use() {
                    LvalueContext::Projection(Mutability::Mutable)
                } else {
                    LvalueContext::Projection(Mutability::Immutable)
                };
            ...
            fn super_typed_const_val(&mut self,
                                     constant: & $($mutability)* TypedConstVal,
                                     location: Location) {
                let TypedConstVal {
                    ref $($mutability)* span,
                    ref $($mutability)* ty,
            ...
            */
            ...
            fn super_ty(&mut self, _ty: & $($mutability)* P<ast::Ty>) {
            }

            /*
            fn super_substs(&mut self, _substs: & $($mutability)* &'tcx Substs) {
            }

            fn super_closure_substs(&mut self,
                                    _substs: & $($mutability)* ClosureSubsts) {
            }
            ...
            */
            ...
            fn visit_location(&mut self, mir: & $($mutability)* Mir, location: Location) {
...
impl LvalueContext {
    ...
    pub fn is_mutating_use(&self) -> bool {
        match *self {
            ...
            LvalueContext::Borrow(Mutability::Mutable) |
            LvalueContext::Projection(Mutability::Mutable) |
            LvalueContext::Drop => true,
            LvalueContext::Inspect |
            LvalueContext::Borrow(Mutability::Immutable) |
            LvalueContext::Projection(Mutability::Immutable) | LvalueContext::Consume |
    ...
    pub fn is_nonmutating_use(&self) -> bool {
        match *self {
            LvalueContext::Inspect | LvalueContext::Borrow(Mutability::Immutable) |
            LvalueContext::Projection(Mutability::Immutable) | LvalueContext::Consume => true,
            LvalueContext::Borrow(Mutability::Mutable) | LvalueContext::Store |
            LvalueContext::Call | LvalueContext::Projection(Mutability::Mutable) |
