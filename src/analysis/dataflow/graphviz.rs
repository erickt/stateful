...
use data_structures::bitslice::bits_to_string;
...
pub trait MirWithFlowState {
    type BD: BitDenotation<Ctxt=MoveDataParamEnv>;
    fn mir(&self) -> &Mir;
    fn analysis_ctxt(&self) -> &<Self::BD as BitDenotation>::Ctxt;
    fn flow_state(&self) -> &DataflowState<Self::BD>;
}

impl<'a, BD> MirWithFlowState for MirBorrowckCtxtPreDataflow<'a, BD>
    where BD: BitDenotation<Ctxt=MoveDataParamEnv>
{
    type BD = BD;
    fn mir(&self) -> &Mir { self.flow_state.mir() }
...
pub fn print_borrowck_graph_to<'a, BD, P>(
    mbcx: &MirBorrowckCtxtPreDataflow<'a, BD>,
    path: &Path,
    render_idx: P)
    -> io::Result<()>
    where BD: BitDenotation<Ctxt=MoveDataParamEnv>,
          P: for <'b> Fn(&'b BD::Ctxt, BD::Idx) -> &'b Debug
{
    ...
    let mut v = Vec::new();
    dot::render(&g, &mut v)?;
    debug!("print_borrowck_graph_to path: {}", path.display());
                         ...
                         -> &'b Debug,
{
    ...
    fn graph_id(&self) -> dot::Id {
        dot::Id::new(format!("graph_for_node"))
            .unwrap()
    }
